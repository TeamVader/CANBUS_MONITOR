                ...
                if (deviceFoundByGuid)
                    ...
                    do
                        ...
                        if (!deviceInformation.hidHandle.IsInvalid)
                            ...
                            if (success)
                                {
                               // MessageBox.Show(string.Format("usbGenericHidCommunication:findTargetDevice() -> Found device with VID {0}, PID {1} and Version number {2}",
                ...
                if (isDeviceDetected)
                    ...
                    if (success)
                        ...
                        if (deviceInformation.readHandle.IsInvalid)
                            {
                           // MessageBox.Show("usbGenericHidCommunication:findTargetDevice() -> Unable to open a readHandle to the device!");
                            }
                        else
                            {
                    ...
                    //  The device wasn't detected.
                      //  MessageBox.Show("usbGenericHidCommunication:findTargetDevice() -> No matching device found!");
                    }
                }
            catch (Exception)
                {
                  //  MessageBox.Show("usbGenericHidCommunication:findTargetDevice() -> EXCEPTION: Unknown - device not found");
                isDeviceDetected = false;
                }
            }
